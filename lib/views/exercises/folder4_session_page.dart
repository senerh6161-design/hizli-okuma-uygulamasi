...
/// Klasör 3'teki gibi tam bir oturum: önce seviyeye göre Ön Metin okunup
/// hız ölçülür ([Folder3ReadingData] — hocanın Klasör 4 için verdiği "İlk
/// ve Son Antreman Metni" belgeleriyle birebir aynı içerik), ardından 10
/// etkinlik tamamlanır, sonra puansız bir Antreman Metni ile ısınılır,
/// ardından genel konu havuzundan FARKLI bir metinle (Son Metin) tekrar
/// hız + D/Y anlama ölçülür, en sonunda Sonuç ekranında önce/sonra
/// karşılaştırması gösterilir.
class Folder4SessionPage extends StatefulWidget {
  const Folder4SessionPage({super.key});

  @override
  State<Folder4SessionPage> createState() => _Folder4SessionPageState();
}

class _Folder4SessionPageState extends State<Folder4SessionPage> {
  ...
  // Etkinliklerden sonra gösterilen, puansız antreman metni — Klasör 3'ün
  // Antreman Metni'yle aynı içerik (hocanın verdiği "İlk Test ve Antreman
  // Metni" belgesinin ortaokul-lise uyarlaması). Kelimeler sayfanın
  // üstünden soldan sağa doğru tek tek gelir; kutu dolunca yeni bir sayfa
  // açılır. Hız ÖLÇÜLMEZ, sadece ısınma amaçlıdır.
  ...
  // Son Metin, genel konu havuzundan (seviyeye özel Folder3ReadingData
  // HARİÇ) rastgele seçiliyor — Ön Metin'den mutlaka farklı bir metin
  // olacak şekilde.
      ...
      children: [
        ...
        Expanded(
          child: Container(
            ...
            // Sağa yaslı boşluk esnemesi (justify) sabit kalsın diye
            // sayfanın TÜM kelimeleri baştan diziliyor; henüz sırası
            // gelmeyenler görünmez (opacity 0) ama yer kaplıyor — böylece
            // kelimeler belirdikçe satırlar kaymıyor, düzen hep aynı kalıyor.
            child: SingleChildScrollView(
              child: Text.rich(
                TextSpan(
                  children: [
